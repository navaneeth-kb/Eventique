    ...
    const fetchEvents = async () => {
      try {
        ...
        querySnapshot.docs.forEach(doc => {
          ...
          // Assuming organiser is stored as email or name
          if (event.organiser === userEmail) {
             const dateKey = event.event_date || event.event_Date;
             if (dateKey) {
            ...
            <>
              ...
              <div className="grid grid-cols-7 gap-1 px-4 pb-4">
                {days.map((day, index) => {
                  ...
                  const hasEvent = !!events[formattedDate] && events[formattedDate].length > 0;
                  ...
                  {selectedDate && events[selectedDate] && events[selectedDate].length > 0 ? (
                    <div className="space-y-3">
                      {events[selectedDate].map((event, index) => (
                        <div key={index} className="flex items-start gap-3 bg-[#f6fcf7] p-4 rounded-xl border border-[#246d8c]/10">
